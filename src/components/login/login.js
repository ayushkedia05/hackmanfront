...
// const [idd,setdd]=useState('');

// const get=useSelector(state=>state.ui.idvisible);
    ...
    Event.preventDefault();
      ...
      const getback=axios.post('http://localhost:3000/api/v1/users/login',viewdata);  
        getback.then(value=>  {
          ...
          //  setdd(value.data.message);
           if(value.data.status==='success')
           console.log('fsfsfs');
           history.push('/products');
           if(value.data.message==='admin')
           {
           dispatch(uiActions.toggle());
           console.log("fsf"); 
          }
            //  dispatch(uiActions.changeid(idd));
...
//           const getit=useSelector(state=>state.ui.idvisible);
// console.log(getit);
